...
const (
	...
	trustChainServerPath = "trustChainServerRSA.pem" // cat of CAs and End-entity Server cert
	trustChainClientPath = "trustChainClientRSA.pem" // cat of CAs and End-entity Client cert
)
...
func (chain *RSAChain) CreateEndEntity(w http.ResponseWriter, endEntity string) {
	...
	// Close trust chain files opened in Root CA certificate creation
	trustChainFile.Close()

	// Create pkcs12 file with certificates and private key using openssl pkcs12
	fileName := fmt.Sprintf("rsa%s%d.p12", endEntity, chain.KeySizeEndEntity)
	absPath, err := exec.LookPath(openssl)
	if err != nil {
		fmt.Printf("openssl LookPath error: %v\n", err)
	} else {
		fmt.Printf("%s is available and %s file will be created.\n", absPath, fileName)
...
func (chain *RSAChain) CreateInterCA(w http.ResponseWriter) {
	...
	if chain.Inter2 != nil {
		...
		fmt.Printf("Wrote %d bytes to %s\n", written, trustChainServerPath)
		// Go to the end of the trust chain file
		trustChainFileServer.Seek(0, io.SeekEnd)
...
func (chain *RSAChain) CreateRootCA(w http.ResponseWriter) {
	...
	// Create trust chain file with CAs and server End-entity cert
