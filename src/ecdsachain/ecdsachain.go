// package ecdsa creates an ECDSA certificate chain consisting of
...
type ECDSAChain struct {
	CurveRoot             elliptic.Curve
	CurveInter1           elliptic.Curve
	CurveInter2           elliptic.Curve
	CurveEndEntity        elliptic.Curve
	Root                  *x509.Certificate
	Inter1                *x509.Certificate
	Inter2                *x509.Certificate
	EndEntity             *x509.Certificate
	rootECDSAkey          *ecdsa.PrivateKey
	intermediate1ECDSAkey *ecdsa.PrivateKey
	intermediate2ECDSAkey *ecdsa.PrivateKey
...
func (chain *ECDSAChain) CreateEndEntity(w http.ResponseWriter) {
	...
	if err != nil {
		log.Fatalf("ecdsa.GenerateKey for End-entity error: %v\n", err)
	}
	// Store the private key in the chain
	chain.endentityECDSAkey = endEntityKey
	...
	if chain.Inter2 != nil {
		parent = chain.Inter2
		private = chain.intermediate2ECDSAkey
	} else if chain.Inter1 != nil {
		parent = chain.Inter1
		private = chain.intermediate1ECDSAkey
	} else {
		parent = chain.Root
		private = chain.rootECDSAkey
	}
	...
	endEntityKeyder, err := x509.MarshalPKCS8PrivateKey(endEntityKey)
	if err != nil {
		log.Fatalf("End-entity key MarshalPKCS8PrivateKey error: %v\n", err)
	}
	...
	// Find ECDSA key size
	var keysize int
	if chain.endentityECDSAkey.PublicKey.Curve == elliptic.P256() {
		keysize = 256
	} else if chain.endentityECDSAkey.PublicKey.Curve == elliptic.P384() {
		keysize = 384
	} else {
		keysize = 512
	}
...
// Create the intermediate CA certificate and private key and store them in the receiver chain
func (chain *ECDSAChain) CreateInterCA(w http.ResponseWriter) {
	...
	if chain.Inter1 != nil {
		...
		// Create ECDSA Intermediate1 CA private key
		inter1CAKey, err := ecdsa.GenerateKey(chain.CurveInter1, rand.Reader)
		if err != nil {
			log.Fatalf("ecdsa.GenerateKey for intermediate1 CA error: %v\n", err)
		}
		// Store the private key in the chain
		chain.intermediate1ECDSAkey = inter1CAKey
		...
		if err != nil {
			log.Fatalf("Create Intermediate1 CA private key file %s error: %v\n", "inter1CAkey.der", err)
		}
		defer f1.Close()

		inter1CAKeyder, err := x509.MarshalPKCS8PrivateKey(inter1CAKey)
		if err != nil {
			log.Fatalf("Intermediate1 CA key MarshallPKCS8PrivateKey error: %v\n", err)
		}
		...
		inter1CACRLder, err := x509.CreateRevocationList(rand.Reader, templCRL, chain.Root, chain.rootECDSAkey)
		if err != nil {
	...
	if chain.Inter2 != nil {
		// Create ECDSA Intermediate2 CA private key
		inter2CAKey, err := ecdsa.GenerateKey(chain.CurveInter2, rand.Reader)
		if err != nil {
			log.Fatalf("ecdsa.GenerateKey for intermediate2 CA error: %v\n", err)
		}
		// Store the private key in the chain
		chain.intermediate2ECDSAkey = inter2CAKey

		inter2CAder, err := x509.CreateCertificate(rand.Reader, chain.Inter2, chain.Inter1, inter2CAKey.Public(),
			chain.intermediate1ECDSAkey)
		if err != nil {
			log.Fatalf("Intermediate1 CreateCertificate error: %v\n", err)
		}
		...
		if err != nil {
			log.Fatalf("Intermediate2 CA ParseCertificates error: %v\n", err)
		}
		// Store the certificate in the receiver chain
		...
		if err != nil {
			log.Fatalf("Create Intermediate2 CA private key file %s error: %v\n", "inter2CAkey.der", err)
		}
		defer f1.Close()

		inter2CAKeyder, err := x509.MarshalPKCS8PrivateKey(inter2CAKey)
		if err != nil {
			log.Fatalf("Intermediate2 CA key MarshallPKCS8PrivateKey error: %v\n", err)
		}
		...
		inter2CACRLder, err := x509.CreateRevocationList(rand.Reader, templCRL, chain.Inter1, chain.intermediate1ECDSAkey)
		if err != nil {
...
func (chain *ECDSAChain) CreateRootCA(w http.ResponseWriter) {
	...
	// Create ECDSA Root CA private key
	rootCAKey, err := ecdsa.GenerateKey(chain.CurveRoot, rand.Reader)
	if err != nil {
		log.Fatalf("ecdsa.GenerateKey for root CA error: %v\n", err)
	}

	// Store the private key in the chain
	chain.rootECDSAkey = rootCAKey
	...
	rootCAKeyder, err := x509.MarshalPKCS8PrivateKey(rootCAKey)
	if err != nil {
		log.Fatalf("Root CA key MarshallPKCS8PrivateKey error: %v\n", err)
	}
...
func GenerateCertChain(chain certchain.CertChain, w http.ResponseWriter) {
	...
	fmt.Fprint(w, `</head>`)
	fmt.Fprint(w, `<body>`)
	fmt.Fprint(w, `<h2>ECDSA Certificate Chain</h2>`)
